...
const errorMessages = {
    ...
    invalidLeftHandSideAssignment: "Invalid left-hand side in assignment",
...
class Parser {
    ...
    parseExpression(parseSequence = true, parseIn = true) {
        let left = this.factorWithUpdateExpr(parseSequence);
        let right = null;
        let lastOpLevel = -1;
        if (left == null) {
            this.unexpected();
        ...
        while (
            ...
            let op = this.token.value;
            let isAssignment = this.isAssignment();
            let start = this.index;
            if (!parseIn && op == "in") {
                break;
            }
            ...
            this.nextToken();
            if (isAssignment) {
                right = this.parseExpression(parseSequence, parseIn);
                left = new AssignmentExpression(left.start, right.end, op, left, right);
                if (left.left.type != "MemberExpression" && left.left.type != "Identifier") {
                    this.throwError(errorMessages.invalidLeftHandSideAssignment);
                }
                break;
            } else {
                right = this.factorWithUpdateExpr(parseSequence);
            }
            if (right == null) {
                this.index = start;
                this.getToken();
    ...
    factorWithUpdateExpr(parseSequence = true) {
        if (this.isUpdate()) {
            let opStart = this.token.start;
            let op = this.token.value;
            this.nextToken();
            ...
            let updateExpr = new UpdateExpression(opStart, factor.end, op, true, factor);
            if (this.isUpdate() || factor.type != "Identifier") {
                this.throwError(errorMessages.invalidLeftHandSide);
            }
            return updateExpr;
        } else {
            ...
            if (this.isUpdate()) {
                let updateExpr = new UpdateExpression(factor.start, this.token.end, this.token.value, false, factor);
                this.nextToken();
                if (this.isUpdate() || factor.type != "Identifier") {
                    this.throwError(errorMessages.invalidLeftHandSide);
                }
    ...
    parseLiteral() {
        ...
        if (
            ...
            if (this.match(Str)) {
                // Do not contains others
                literal.value = this.token.value.substring(1, this.token.value.length - 1);
            }
            if (this.match(Num)) {
    ...
    match(tokenKind) {
        if (this.token != null) {
            const kind = this.token.tokenKind;
            if (kind === tokenKind || kind.tag == tokenKind.tag) {
                return true;
            }
        }
        return false;
